    ...
    <div class="row align-items-center">
        <div class="border-0 mb-4">
            <div
                ...
                <h3 class="fw-bold mb-0">Service Faq Add</h3>
            </div>
        </div>
    </div>

    <div class="card-body">
        <form method="post" enctype="multipart/form-data" action="{{ route('servicefaq.store') }}">
            @csrf
            <div class="col-xl-12 col-lg-8">
                <div class="card mb-3 p-3">
                    <div class="card-header py-3 p-0 d-flex justify-content-between bg-transparent border-bottom-0">
                        <h6 class="mb-0 fw-bold">Service Faq Information</h6>
                    </div>
                    <div class="row g-3 align-items-center">
                        <div class="col-sm-12">
                            <label for="service_id" class="form-label">Please Select The Service Name</label>
                            <select class="form-select" id="industry_id" name="industry_id" required>
                                <option value="" disabled selected>Select Service</option>
                                @foreach ($servicelist as $service)
                                <option value="{{ $service->id }}">{{ $service->service_header }}</option>
                                @endforeach
                            </select>
                        </div>
